...
class MusicProvider extends ChangeNotifier {
  ...
  /// All unique parent folders containing music files
    ...
    for (final s in _allSongs) {
      ...
      if (parts.length > 2) {
        ...
        for (final p in parts) {
          ...
          building += '/$p';
          if (building.split('/').length > 4) {
            folders.add(building);
          }
        }
      }
    }
  ...
  Future<void> init() async {
    ...
    try {
      ...
      // Delay after first permission grant to let MediaStore index
  ...
  Future<void> setRootFolder(String? path) async {
    ...
    try {
      ...
      if (path != null && path.isNotEmpty) {
        await prefs.setString('cyberplayer_root_folder', path);
      } else {
        await prefs.remove('cyberplayer_root_folder');
      }
    } catch (_) {}
    notifyListeners();
  }
  ...
  Future<void> rescan() async {
    ...
    try {
      ...
      _error = 'SCAN_ERROR: $e'; _isLoading = false; notifyListeners();
    }
  }
