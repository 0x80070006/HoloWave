...
class MusicProvider extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    try {
      try {
        ...
        _rootFolder = prefs.getString('holowave_root_folder');
      } catch (_) {}
  ...
  Future<void> setRootFolder(String? path) async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      if (path != null && path.isNotEmpty) await prefs.setString('holowave_root_folder', path);
      else await prefs.remove('holowave_root_folder');
    } catch (_) {}
    notifyListeners();
  }
  ...
  Future<void> setPlaylistCover(int pi, String? path) async { await playlistService.setCoverImage(pi, path); notifyListeners(); }
